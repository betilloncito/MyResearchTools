function varargout = HallEffect_Density_Analysis(varargin)
...
if(nargin==0)
    ...
    DataTable{1,2} = 1e-6;
    DataTable{2,1} = 'Gain (Vhall) [V/V]:';
    DataTable{2,2} = 100;
    DataTable{3,1} = 'Start at pt INDEX:';
    DataTable{3,2} = 1;
...
%         '1.5T (Probe: 4-6)' '2T (Probe: 2-12)' '2T (Probe: 4-6)'};
    ...
    for INDEX=1:length(MatrixData_All)
        plot(Vg_vector(:,INDEX),SheetDensity(:,INDEX),'*-');grid on;hold on;
    end
