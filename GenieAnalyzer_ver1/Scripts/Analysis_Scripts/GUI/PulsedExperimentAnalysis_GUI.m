function varargout = PulsedExperimentAnalysis_GUI(varargin)
% PULSEDEXPERIMENTANALYSIS_GUI MATLAB code for PulsedExperimentAnalysis_GUI.fig
%      PULSEDEXPERIMENTANALYSIS_GUI, by itself, creates a new PULSEDEXPERIMENTANALYSIS_GUI or raises the existing
%      singleton*.
%
%      H = PULSEDEXPERIMENTANALYSIS_GUI returns the handle to a new PULSEDEXPERIMENTANALYSIS_GUI or the handle to
%      the existing singleton*.
%
%      PULSEDEXPERIMENTANALYSIS_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PULSEDEXPERIMENTANALYSIS_GUI.M with the given input arguments.
%
%      PULSEDEXPERIMENTANALYSIS_GUI('Property','Value',...) creates a new PULSEDEXPERIMENTANALYSIS_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PulsedExperimentAnalysis_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PulsedExperimentAnalysis_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PulsedExperimentAnalysis_GUI

% Last Modified by GUIDE v2.5 07-Aug-2018 17:02:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PulsedExperimentAnalysis_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @PulsedExperimentAnalysis_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PulsedExperimentAnalysis_GUI is made visible.
function PulsedExperimentAnalysis_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PulsedExperimentAnalysis_GUI (see VARARGIN)

% Choose default command line output for PulsedExperimentAnalysis_GUI
...
handles.VarDataTable = [{'Vgate:'},{'Vplg_T'}; {'Osc_Y [volt]:'},{'Osc_wave'};...
    ...
    {'Delay (s)'},{'0'}];
handles.FitDataParamTable = [{'Base Pt1:'},{'1'}; {'Ref MAX Value:'},{'0.019'}; {'Ref MIN Value:'},{'0.016'};...
...
% UIWAIT makes PulsedExperiment_Analysis wait for user response (see UIRESUME)
% uiwait(handles.PulsedExperiment_Figure);



% --- Outputs from this function are returned to the command line.
function varargout = PulsedExperimentAnalysis_GUI_OutputFcn(hObject, eventdata, handles) 
...
Ref_Max = str2double(cell2mat(FitTable(2,2)));
Ref_Min = str2double(cell2mat(FitTable(3,2)));
...
n4 = size(MatrixData,3);%Vgate

OscY = MatrixData(:,OscY_index,:);
size(OscY);
handles.OscY = reshape(OscY,n1,n4);
...
OscX = MatrixData(:,OscX_index,1);
handles.OscX = reshape(OscX,n1,1);
size(handles.OscX)

Vgate = MatrixData(1,Vgate_index,:);
handles.Vgate_nonrepeat = reshape(Vgate,n4,1);
handles.Vgate = handles.Vgate_nonrepeat;
...
% figure;
for i=1:size(handles.OscY,2)
    RefLevel_Avg = mean(handles.OscY(index_xmin:index_xmax,i));
    handles.OscY(:,i) = handles.OscY(:,i) - RefLevel_Avg;
%     plot(handles.OscX,handles.OscY(:,i));hold on; grid on;
end
...
delete(child);
set(handles.PulsedExperiment_Figure,'CurrentAxes',handles.axes1);
surf(handles.OscX,handles.Vgate,transpose(handles.OscY),'EdgeAlpha',0);
view(2);
...
% delete(child);
% 
% set(handles.PulsedExperiment_Figure,'CurrentAxes',handles.axes2);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(0,'CurrentFigure',handles.PulsedExperiment_Figure);
...
elseif(get(handles.AxesNum3Radiobutton,'Value')==1)
    set(handles.PulsedExperiment_Figure,'CurrentAxes',handles.axes3);
end
...
% hObject    handle to ApplyMathScriptPushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
set(handles.PulsedExperiment_Figure,'CurrentAxes',axesHandle);
...
% hObject    handle to OpenMathScriptPushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
NowDir = cd;
...
% --- Executes when user attempts to close PulsedExperiment_Figure.
function PulsedExperiment_Figure_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to PulsedExperiment_Figure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
